document.addEventListener("DOMContentLoaded", () => {
    const target = document.querySelector(".bvvg21"); // Sélectionnez l'élément à animer
      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            ...
            entry.target.classList.remove("hidden-animation4");
          } else {
            // Retire l'animation lorsque l'élément sort de l'écran
            entry.target.classList.add("hidden-animation4");
